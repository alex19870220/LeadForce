...
use ShutterProxy;
use Acme\Scraper\Exceptions\ShutterScraperException;
use Acme\Scraper\RollingCurl\RollingCurl;
use Acme\Scraper\RollingCurl\Request;
use Proxy;
...
class ShutterScraper extends RollingCurl {
	...
	/**
	 * The ShutterProxy proxy handler
	 *
	 * @var ShutterProxy
	 */
	protected $ShutterProxy;
	 ...
	 *
	 * @throws ShutterScraperException
	...
	public function execute($window_size = null){
		...
		if($window_size == null){
			...
			throw new ShutterScraperException(" (!) Wrong threads amount in execute():\t$window_size");
		}
	...
	*/
	...
	{
		// Grab random proxy
		$proxy = ShutterProxy::getProxy();
		
		// Sleep for the Proxy cooldown time if there's no Proxies
		if(! $proxy)
		{
			sleep(ShutterProxy::getProxyTimeout());
		...
		self::add_debug_msg(" + Fetched proxy ->\t {$proxy->ip}:{$proxy->port}:{$proxy->username}:{$proxy->password}");

		return $options;
	}

	/**
	 * Update last used Proxy's data
	 *
	 * @param  ShutterRequest $request
	 * @return bool
	 */
	public function updateProxy(ShutterRequest $request)
	{
		// Response Info
		...
		ShutterProxy::updateProxyStats($ip, $data);
