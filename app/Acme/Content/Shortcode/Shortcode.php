...
class Shortcode implements Countable
{
	...
	/**
	 * @var array $shortcodeCache
	 */
	protected $shortcodeCache = [];
	...
	public function compile($content)
	{
		...
		// Clockwork::info('Compiling Shortcodes');

		$pattern = $this->getRegex();

		$output = preg_replace_callback("/{$pattern}/s", [&$this, 'render'], $content);
	...
	{
		// dd($matches);
		if(! isset($this->shortcodeCache[$matches[0]]))
		{
			// Clockwork::startEvent('parse_shortcode' . $matches[0], 'Parsing ' . $matches[0]);
			...
			$this->shortcodeCache[$matches[0]] = call_user_func_array($callback, $params);

			// Clockwork::endEvent('parse_shortcode' . $matches[0]);

			return $this->shortcodeCache[$matches[0]];
		}
	...
	protected function getCallback($name)
	{
		...
		// echo 'TIME: ' . round(($time_end - $time_start), 6) . '<br />';
